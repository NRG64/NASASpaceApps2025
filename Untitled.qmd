...
Shark_states <- Full_Shark_Data %>%
  ...
  mutate(
    ...
    time_diff_h = as.numeric(difftime(next_time, datetime, units = "hours")),
    dist_m = distHaversine(cbind(longitude, latitude), cbind(next_lon, next_lat)),
    speed_mph = dist_m / time_diff_h,
    state = case_when(
      time_diff_h > 0 & time_diff_h <= max_gap_h & speed_mph < 50 ~ "resting",
      time_diff_h > 0 & time_diff_h <= max_gap_h & speed_mph < 500 ~ "foraging",
      time_diff_h > 0 & time_diff_h <= max_gap_h ~ "migrating",
      TRUE ~ NA_character_
    ),
...
transition_counts <- Shark_states %>%
